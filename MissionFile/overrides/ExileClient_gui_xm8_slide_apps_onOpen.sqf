...
if (_popTabsValue > 999) then{_popTabsValue = format ["%1k", floor (_popTabsValue / 1000)];};
...
if (_respectValue > 999) then{_respectValue = format ["%1k", floor (_respectValue / 1000)];};
...
[_this, _txt] spawn {sleep 5; _this select 0 ctrlSetStructuredText parseText (format [_this select 1, missionNamespace getVariable ["scratchieCount", 0], "Scratchies"]);};};
